...
N_INPUTS = 4  # ground_dist, dx, dy, velocity
...
def create_random_genome() -> Genome:
    ...
    genome.append(randint(1, MAX_HIDDEN_LAYERS)) 
    genome.append(
...
def evaluate_genome(genome: Genome) -> tuple[float, float]:
    ...
    for _ in range(EPOCHS):
        ...
        while True:
            ground_dist, dx, dy, vel = game.get_state()   
            game_state = torch.tensor((ground_dist, dx, dy, vel), dtype=torch.float32).unsqueeze(0)
    ...
    return (fitness, float(actual_game_score))
...
def train(save_path: Path):
    ...
    deap_creator.create("FitnessMax", deap_base.Fitness, weights=(1.0, 1.0))
    ...
    stats_score = deap_tools.Statistics(lambda ind: ind.fitness.values[1])
    ...
    for i, ind in enumerate(hof):
        print(f"Rank {i+1} | Algorithmic Fitness: {ind.fitness.values[0]:.2f} | Actual Game Score: {ind.fitness.values[1]:.1f}")
    ...
    torch.save(
        {
            ...
            "actual_score": hof[0].fitness.values[1],
